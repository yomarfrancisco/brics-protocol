...
async function main() {
  ...
  // Settle claim
  const settleTx = await ic.connect(deployer).settleClaim(1, 1, user.address);
  ...
  const navAtStrike = await ic.getNavAtStrike(1); // window 1
  const carriedOver = await ic.getCarryoverAmount(1, 1); // window 1, claim 1
